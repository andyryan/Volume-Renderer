...
inline int Vector3::indexOfMinComponent() const{
	int index = 0;
	float temp = e[0];
	if (temp > e[1]) { temp = e[1]; index = 1; }
	if (temp > e[2]) { temp = e[2]; index = 2; }
...
inline int Vector3::indexOfMinAbsComponent() const{
	int index = 0;
	float temp = fabs(e[0]);
	if (temp > fabs(e[1])) { temp = fabs(e[1]); index = 1; }
	if (temp > fabs(e[2])) { temp = fabs(e[2]); index = 1; }
